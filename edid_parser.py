...
def parse(blob):
    ...
    def read_litendian32(offset):
        return ((ord(blob[offset + 3]) << 24) |
                (ord(blob[offset + 2]) << 16) |
                (ord(blob[offset + 1]) <<  8) |
                 ord(blob[offset])
               )
    def read_bigendian24(offset):
        return ((ord(blob[offset]) << 16) |
                (ord(blob[offset + 1]) << 8) |
                (ord(blob[offset + 2]))
               )
    ...
    week = ord(blob[WEEK_OFFSET])
    year = ord(blob[YEAR_OFFSET]) + 1990
    print (week, year)

    widthcm = ord(blob[WIDTH_CM_OFFSET])
    heightcm = ord(blob[HEIGHT_CM_OFFSET])

    prefferedResIndex = (ord(blob[FEATURES24_OFFSET]) & 0x2) >> 1
    ...
    for word in grouper(timingBlock, 2):
        if (ord(word[0]), ord(word[0])) <> (1, 1):
            Xresolution = (ord(word[0]) + 31) * 8
            ind = (ord(word[1]) & 0xC0) >> 6
            ratio = ratios[ind]
